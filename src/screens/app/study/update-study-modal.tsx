...
import { Scripture, StudyType, StudyTypeRequest } from '@/utils/study-types';
import { supabase } from '@/utils/lib/superbase';
import { databaseTables } from '@/constants/db-tables';
import { updateStudyState } from '@/redux/app/study-slice';
...
export default function UpdateStudyModal({onClose, study}:componentProp) {
  // const router = useRouter();
  
  // State for form data
  const dispatch = useAppDispatch();
  const [title, setTitle] = useState(study.title || '');
  const [month, setMonth] = useState(study.month || MONTHS[new Date().getMonth()]); // Default to current month
  const [introduction, setIntroduction] = useState(study.introduction || '');
  const [scriptures, setScriptures] = useState<Scripture[]>(study.bible_text || [{ scripture: '', content: '' }]);
  ...
  const updateStudy = async () => {
    try {
        setIsSubmitting(true);
        const {data, error} = await supabase.from(databaseTables.study).update(studyRequestPayload).eq('id', study.id).select("*").single();
        if (error) {
            Alert.alert(error.message);
        }else{
            handleOnClose();
            Alert.alert('Update was Successful');
            dispatch(updateStudyState(data));
        }
  ...
  return (
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Update Bible Study</Text>
        <TouchableOpacity 
